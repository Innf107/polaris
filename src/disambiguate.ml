...
let builder = { unifs = StringMap.empty; skolems = StringMap.empty }
            ...
            let state =
              {
                state with
                unifs =
                  StringMap.update name.name
                    (add_entry (Typeref.get_unique typeref) name)
                    state.unifs;
              }
            in
            (ty, state)
            ...
            let state =
              {
                state with
                skolems =
                  StringMap.update name.name (add_entry unique name)
                    state.skolems;
              }
            in
            (ty, state)
...
let unify_context (type1, type2) builder = builder |> ty type1 |> ty type2
...
let types types disambiguate = List.fold_right ty types disambiguate
        ...
        begin
          fun typeref name ->
            let suffix =
              match
                Option.bind
                  (StringMap.find_opt name.name disambiguate.unifs)
                  (UniqueMap.find_opt (Typeref.get_unique typeref))
              with
              | None ->
                  (Lazy.force default_pretty_type_config).pretty_unif typeref
                    name
              | Some 0 -> ""
            ...
            | Bound ty ->
                if Config.print_subst_unif_vars () then
                  ...
                  ^ "]"
                else pretty_type disambiguate ty
        end;
      pretty_skol =
        begin
          fun unique level name ->
            let suffix =
              match
                Option.bind
                  (StringMap.find_opt name.name disambiguate.skolems)
                  (UniqueMap.find_opt unique)
              with
              | None ->
                  (Lazy.force default_pretty_type_config).pretty_skol unique
                    level name
              | Some 0 -> ""
              | Some i -> string_of_int i
            in
