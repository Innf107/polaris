  ...
  if enable_color then
    {
      ...
      number =
        (fun number -> "\x1b[1m\x1b[93m" ^ string_of_int number ^ "\x1b[0m");
      ...
      error =
        (fun message ->
          "\x1b[20m\x1b[31m\x1b[38;2;255;0;0m" ^ message ^ "\x1b[0m");
      warning =
        (fun message -> "\x1b[35m\x1b[38;2;192;0;192m" ^ message ^ "\x1b[0m");
...
let extract_line index in_channel =
...
let extract_source_fragment loc in_channel text_style error_or_warning =
  ...
  let line_at_error, line_after =
    if loc.end_line > loc.start_line then
      ...
      ( String.sub line (loc.start_col - 1)
          (String.length line - (loc.start_col - 1)),
        "" )
    else
      ( String.sub line (loc.start_col - 1) (loc.end_col - loc.start_col),
        String.sub line (loc.end_col - 1)
  ...
  let underline =
    ...
    ^ text_style.error underline_string
  in
  ("", line_before ^ color line_at_error ^ line_after, underline)
