...
module Template = struct
  ...
  let rec pretty = function
    ...
    | LetTypeSeq (_, varname, params, ty) ->
        ...
        ^ String.concat ", " (List.map pretty_name params)
        ^ ") = " ^ pretty_type ty
    | LetClassSeq (_, classname, params, methods) ->
        "class " ^ pretty_name classname ^ "("
        ^ String.concat ", " (List.map pretty_name params)
        ^ ") {\n"
        ^ String.concat "\n"
            (List.map
                  ...
                  let methods, state =
                    traverse_list
                      (fun state (ext, method_name, patterns, expr) ->
                        let ext, state =
                          InstanceMethodExt.traverse_ty state self#traverse_type
                            ext
                        in
                        let method_name, state =
                          self#traverse_name state method_name
                        in
                        let patterns, state =
                          traverse_list self#traverse_pattern state patterns
                        in
