...
let rec parse_rename_typecheck : driver_options
                              ...
                              -> Types.global_env
                              -> (Typed.header * Typed.expr list * RenameScope.t * Types.global_env, Error.t) result
  ...
  Error.handle_errors (fun err -> Error err) begin fun () ->
    ...
    let header, ast = 
      ...
      | exception Parser.Error -> 
        ...
        raise (Parserprelude.ParseError (Loc.from_pos start_pos end_pos, "Parse error")) 
      | res -> res
    in
    ...
    let items_for_exports = List.map (fun (filename, path) -> 
        (filename, Error.as_exn (parse_rename_typecheck {options with filename=path} (Lexing.from_channel (open_in path)) RenameScope.empty Types.empty_env))
      ) imported_files in
  ...
  = fun options lexbuf env scope ?check_or_infer_top_level type_env ~fs ~mgr ->
    Error.handle_errors (fun err -> Error err) begin fun () ->
      ...
      let* renamed_header, renamed, new_scope, new_type_env = 
        parse_rename_typecheck options lexbuf scope ?check_or_infer_top_level type_env in
      
      trace_driver (lazy "Evaluating...");
      let env = Eval.eval_header env renamed_header in
      let context = match check_or_infer_top_level with
        | Some `Check | None -> `Statement
        | Some `Infer -> `Expr
      in
      Eio.Switch.run begin fun switch ->
        let res, new_env = Eval.eval_seq_state ~cap:{ switch; fs; mgr } context env renamed in
        Ok (res, new_env, new_scope, new_type_env)
      end
    end

let run (options : driver_options) (lexbuf : Lexing.lexbuf) ~fs ~mgr : (value, Error.t) result =
  let result = run_env ~fs ~mgr options lexbuf (Eval.make_eval_env options.argv) RenameScope.empty Types.empty_env in
