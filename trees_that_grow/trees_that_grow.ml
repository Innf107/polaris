...
let instantiate_ttg : structure -> structure -> structure =
 fun template instantiation ->
  let collect_item_traversal =
    object
      inherit
        [structure_item list * core_type_desc LabelMap.t] Ast_traverse.fold as super
        ...
        | {
         ...
         pstr_desc =
           Pstr_type
             ( _,
               [
                 { ptype_name; ptype_manifest = Some { ptyp_desc = ty; _ }; _ };
               ] );
        } ->
  ...
  let instantiation_items, instantiation_types =
    collect_item_traversal#structure instantiation ([], LabelMap.empty)
  in

  let replace_traversal =
    object
      ...
      method! core_type_desc ty =
        ...
        match ty with
        | Ptyp_constr (name, _) as ty -> begin
            match name.txt with
            | Lident label -> begin
                match LabelMap.find_opt label instantiation_types with
  ...
  let separate_template_modules = function
    | { pstr_loc; pstr_desc = Pstr_module template_mod } as item ->
        ...
        if List.exists is_template_attribute template_mod.pmb_attributes then
          ...
          | Some name -> begin
              match template_mod.pmb_expr with
              ...
              | { pmod_loc = loc; _ } ->
                  Either.Right
                    [%stri
                      [%error
                        "ttg templates need to be named structure literals"]]
            end
          | None ->
              let loc = template_mod.pmb_name.loc in
              Either.Right
                [%stri
                  [%error "ttg_template needs to be used with a named module"]]
        else Either.Right item
    | item -> Either.Right item
  in
  let template_modules, new_structure =
    List.partition_map separate_template_modules structure
  in
    ...
    | { pstr_loc; pstr_desc = Pstr_module replacement_mod } as item ->
        ...
        if List.exists is_pass_attribute replacement_mod.pmb_attributes then
           ...
           pmod_desc =
             Pmod_apply
               ( {
                 ...
                 { pmod_desc = Pmod_structure instantiation; _ } );
           pmod_loc;
           _;
          } -> begin
              match LabelMap.find_opt template_name template_modules with
              | Some template_expr ->
          ...
          | { pmod_loc = loc; _ } ->
              [%stri
                [%error
                  "Modules with a ttg_pass attribute need to contain a functor \
                   application with a non-qualified name from the same module \
                   and a structure literal"]]
        else item
    | item -> item
  in
  List.map replace_template new_structure

let () = Driver.register_transformation ~impl:ttg_impl "trees_that_grow"
