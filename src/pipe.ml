...
let rec compose_pipe ~mgr ~sw ~env (stdin : #Eio.Flow.source option)
    (stdout : #Eio.Flow.sink option) = function
...
let compose ~sw ~mgr ~env progs : Eio.Process.t =
  compose_pipe ~sw ~mgr ~env None None progs
