...
let _request_category, trace_request = Trace.make ~flag:"request" ~prefix:"REQUEST"
let _response_category, trace_response = Trace.make ~flag:"response" ~prefix:"RESPONSE"
...
let output_newline_cr out_channel = 
  ...
  let rec parse known_length =
    ...
    | Some line ->
      ...
      | Some content_length_string ->
        ...
        | None -> raise (Failure ("Error parsing request: Invalid content length: '" ^ content_length_string ^ "'"))
